...
export default function WebsiteMaker() {
  ...
  // types: "error", "warning", "rate-limit", "network"
...
`.trim();
    ...
    try {
      ...
      const newHistory = [newSite, ...history].slice(0, 5);
      ...
      if (data.finishReason === "MAX_TOKENS") {
        setStatus({
          type: "warning",
          message: "Website generation was cut due to token limits."
        });
      }
      ...
      if (msg.includes("rate") || msg.includes("quota") || msg.includes("429")) {
        ...
        setStatus({
          type: "network",
          message: "Network issue or API not responding."
        });
      }
      else {
        setStatus({
    ...
    <div className="flex h-screen bg-[#050505] text-gray-200 overflow-hidden font-sans">
      {status && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#0a0a0b] border border-gray-800 rounded-2xl p-6 max-w-sm w-full text-center shadow-2xl">

            <h2 className="text-lg font-semibold mb-2 text-white">
              {status.type === "warning" && "⚠️ Incomplete Generation"}
              {status.type === "error" && "❌ Error"}
              {status.type === "network" && "🌐 Network Issue"}
              {status.type === "rate-limit" && "⏳ Rate Limit"}
            </h2>
              ...
              <div className="grid grid-cols-1 gap-2">
                {LAYOUTS.map(l => (
                  <button
                    ...
                    className={`text-left px-4 py-3 rounded-xl border text-sm transition-all ${layout === l.id
              ...
              <div className="grid grid-cols-2 gap-2">
                {THEMES.map(t => (
                  <button
                    ...
                    className={`text-left px-4 py-3 rounded-xl border text-xs transition-all ${theme === t.id
              ...
              >
                ...
                <div className="w-full space-y-3 mt-4">
                  {LOADING_STEPS.map((step, idx) => (
                    <div
                      key={idx}
                      className={`text-sm flex items-center gap-3 transition-opacity duration-300 ${idx === progressStep ? "text-blue-400 font-medium scale-105 origin-left" :
